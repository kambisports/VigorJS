define (require) ->
	...
	class GroupsRepository extends ServiceRepository
		...
		services:
			'all': GroupsService
			'highlighted': GroupsHighlightedService
		...
		initialize: ->
			...
			# Live events belongs to groups that might not be available in the prematch groups
			LiveEventsService.on LiveEventsService.GROUPS_RECEIVED, @_onLiveGroupsReceived
			super
		...
		_onLiveGroupsReceived: (flatGroupModels) =>
			@set flatGroupModels, { remove: false }

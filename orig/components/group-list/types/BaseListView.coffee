define (require) ->
	...
	class BaseListView extends ComponentView
		...
		renderStaticContent: ->
			...
			@_$showMore = @$el.find '.modularized__js-event-group-show-more'
			@_$showMore.hide()
		...
		_renderGroups: (groups) ->
			if groups and groups.length
				...
				@$el.find('.modularized__js-event-groups__list').html frag
		...
		_setExpandState: (nrOfGroups = 0) =>
			...
			forceExpand = @viewModel.forceExpand

			if showMoreClicked or forceExpand
				expanded = yes
				showMore = no
